%%% Q2 EM Algorithm, updating CCPs from the model (rest is the same)
...
while max(abs(params-params0))>1e-3 && count <= 200
    ...
    % update CCPs using conditional likelihoods
    % under stationarity, we stack all columns conditional likelihoods
    % stacked it rowwise, but order doesnt really matter

    like_nst_Stack = reshape(permute(like_nst, [2, 1, 3]), [5000*5, 2]);
