...
S2(:,1)=fminunc(Params_LL,0.5*ones(14,1),OptimOptions);
%Two errors
Eps=[-1,1]';
Params_LL= @(Params) LL(9,LY1, Y, X1,X1t, Params(1:4),Params(5:8), Params(9:12),Params(13),Alpha, sigma, Params(14),Eps);
S2(:,2)=fminunc(Params_LL,0.5*ones(14,1),OptimOptions);
%One error
Eps=[-1.3,0,1.3]';
Params_LL= @(Params) LL(9,LY1, Y, X1,X1t, Params(1:4),Params(5:8), Params(9:12),Params(13),Alpha, sigma, Params(14),Eps);
S2(:,3)=fminunc(Params_LL,0.5*ones(14,1),OptimOptions);
%Two errors
Eps=normrnd(0,sigma,15,1);
Params_LL= @(Params) LL(9,LY1, Y, X1,X1t, Params(1:4),Params(5:8), Params(9:12),Params(13),Alpha, sigma, Params(14),Eps);
S2(:,4)=fminunc(Params_LL,0.5*ones(14,1),OptimOptions);
...
for tstart=9:-1:6
    ...
    %One error
    Eps=0;
    Params_LL= @(Params) LL(tstart,LY1, Y, X1,X1t, Params(1:4),Params(5:8), Params(9:12),Params(13),Alpha, sigma, Params(14),Eps);
    S(:,2*(ind-1)+1)=fminunc(Params_LL,0.5*ones(14,1),OptimOptions);
    %Two errors
    Eps=[-1,1]';
    Params_LL= @(Params) LL(tstart,LY1, Y, X1,X1t, Params(1:4),Params(5:8), Params(9:12),Params(13),Alpha, sigma, Params(14),Eps);
    S(:,2*ind)=fminunc(Params_LL,0.5*ones(14,1),OptimOptions);
end
