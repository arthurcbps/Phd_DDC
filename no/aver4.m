...
function Vvalue=V(t,s,X1,X1t,gamma1,gamma2, delta,c,Alpha, sigma, beta,Eps)
    ...
    if t==10
        v(:,2:5)=X1tUpdt.*repmat(delta',n,1)+repmat(X1(:,1),1,4).*repmat(gamma1',n,1)+repmat(X1(:,2),1,4).*repmat(gamma2',n,1)-c*(repmat(1:4,n,1)~=s);
        Vvalue=v;
    else
        v(:,2:5)=X1tUpdt.*repmat(delta',n,1)+repmat(X1(:,1),1,4).*repmat(gamma1',n,1)+repmat(X1(:,2),1,4).*repmat(gamma2',n,1)-c*(repmat(1:4,n,1)~=s);
        for j=2:5
            v(:,j)=v(:,j)+beta*log(sum(exp(V(t+1,j-1,X1,X1tUpdt,gamma1,gamma2, delta,c,Alpha, sigma, beta)),2))+0.57721;
        end
        Vvalue=v;
    end
end
